...
import { User, Users, Info, HelpCircle } from 'lucide-react';
...
const LobbyScreen = ({ setGameMode, setScreen, userId, debugMode }) => {
    ...
    const handleModeSelect = async (mode) => {
        console.log("🎯 [DEBUG] Mode selected:", { mode, userId, debugMode });
        ...
        console.log("🔍 [DEBUG] Searching for existing games:", { mode, requiredPlayerCount });

        // デバッグモードの場合、待機中のゲームをスキップして新規作成
        if (!debugMode) {
            // 待機中のゲームを検索
            const q = query(gamesRef, where("mode", "==", mode), where("status", "==", "waiting"));
        ...
        console.log("💾 [DEBUG] Storing game info in localStorage:", { gameIdToJoin });
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 to-slate-700 text-white p-4">
            ...
            <div className="bg-slate-800 p-8 rounded-xl shadow-2xl w-full max-w-md">
                ...
                <div className="space-y-6">
                    <button onClick={() => handleModeSelect('2player')}
                    ...
                    <button onClick={() => handleModeSelect('4player')}
            ...
            <footer className="mt-12 text-center text-slate-400 space-x-4">
                 <button onClick={() => {
                    const notificationArea = document.getElementById('notification-area');
                    if (notificationArea) {
                        notificationArea.innerHTML = "<strong>遊び方：</strong><br>各モードのルールに従ってプレイしてください。";
